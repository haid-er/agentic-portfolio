...
export const notes: DemoNotes = {
  howItWorks:
    'An homage to a cricket simulation written in pure C, rebuilt for the browser in the same spirit. The match is one plain, serialisable state object (the "structs"), and every ball goes through a single pure update function, like a game loop. Outcomes come from a lookup table of weights indexed by delivery and shot, adjusted by your timing and the opposition level, and drawn with the ANSI C sample rand(), a 32-bit linear congruential generator seeded once by srand(seed). The same seed and the same choices therefore replay the same match. Bat by reading the delivery at the bowler’s release (its length and line show on the pitch, though stronger bowlers disguise some), then playing the shot that suits it as the ball reaches the hit zone (early, good, perfect or late), or bowl by picking deliveries while the CPU batter reads the chase. The engine prints console lines as it goes, which is exactly what terminal mode shows.',
  limits: [
